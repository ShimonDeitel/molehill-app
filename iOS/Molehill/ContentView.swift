...
struct ContentView: View {
    ...
    @State private var editingEntry: PestEntry?
        ...
        NavigationStack {
            ZStack {
                ...
                if store.entries.isEmpty {
                    ContentUnavailableView("No Pest Log Entries Yet", systemImage: "square.and.pencil", description: Text("Tap + to add your first entry."))
                        ...
                        ForEach(store.entries) { entry in
                            Button {
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    Text(entry.pestType.isEmpty ? "Untitled" : entry.pestType)
...
struct EntryEditorView: View {
    ...
    @State private var draft: PestEntry
    @State private var draftDate: Date = Date()

    init(entry: PestEntry?) {
        self.entry = entry
        _draft = State(initialValue: entry ?? PestEntry())
    ...
    var body: some View {
        NavigationStack {
            ZStack {
                Theme.background.ignoresSafeArea()
                Form {
                TextField("Pest Type", text: $draft.pestType)
